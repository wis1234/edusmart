    ...
    <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">Edit Schedule</h1>
                    <p class="text-gray-500 dark:text-gray-300">Update schedule details</p>
                </div>
            </div>
            <div class="flex gap-3">
                <a href="{{ route('calendars.show', $calendar) }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 font-semibold shadow hover:bg-gray-200 dark:hover:bg-gray-700 transition">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="teacher_id" id="teacher_id" class="w-full rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-indigo-500 @error('teacher_id') border-red-500 @enderror">
                            ...
                            @foreach($teachers as $teacher)
                                <option value="{{ $teacher->id }}" @if(old('teacher_id', $calendar->teacher_id) == $teacher->id) selected @endif>{{ $teacher->name ?? ($teacher->teacher_firstname . ' ' . $teacher->teacher_lastname) }}</option>
                            @endforeach
                        </select>
                        @error('teacher_id')
                    ...
                    <div>
                        ...
                        <select name="class_room_id" id="class_room_id" class="w-full rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-indigo-500 @error('class_room_id') border-red-500 @enderror">
                            ...
                            @foreach($classRooms as $classRoom)
                                <option value="{{ $classRoom->id }}" @if(old('class_room_id', $calendar->class_room_id) == $classRoom->id) selected @endif>{{ $classRoom->name }}</option>
                            @endforeach
                        </select>
                        @error('class_room_id')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select name="subject_id" id="subject_id" class="w-full rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-indigo-500 @error('subject_id') border-red-500 @enderror">
                            ...
                            @foreach($subjects as $subject)
                                <option value="{{ $subject->id }}" @if(old('subject_id', $calendar->subject_id) == $subject->id) selected @endif>{{ $subject->name }}</option>
                            @endforeach
                        </select>
                        @error('subject_id')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select name="school_id" id="school_id" class="w-full rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-indigo-500 @error('school_id') border-red-500 @enderror">
                            ...
                            @foreach($schools as $school)
                                <option value="{{ $school->id }}" @if(old('school_id', $calendar->school_id) == $school->id) selected @endif>{{ $school->name }}</option>
                            @endforeach
                        </select>
                        @error('school_id')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select name="academic_year" id="academic_year" class="w-full rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-indigo-500 @error('academic_year') border-red-500 @enderror">
                            ...
                            <option value="2023-2024" @if(old('academic_year', $calendar->academic_year) == '2023-2024') selected @endif>2023-2024</option>
                            <option value="2024-2025" @if(old('academic_year', $calendar->academic_year) == '2024-2025') selected @endif>2024-2025</option>
                            <option value="2025-2026" @if(old('academic_year', $calendar->academic_year) == '2025-2026') selected @endif>2025-2026</option>
                        </select>
                        @error('academic_year')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" name="week_number" id="week_number" min="1" max="52" value="{{ old('week_number', $calendar->week_number) }}" class="w-full rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-indigo-500 @error('week_number') border-red-500 @enderror" placeholder="Enter week number">
                        @error('week_number')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="day-schedule transition-all duration-300 ease-in-out transform scale-95 opacity-0 absolute w-full max-w-lg mx-auto z-10" id="schedule-{{ $day }}" style="display: none;">
                            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8 flex flex-col items-center w-full max-w-lg mx-auto border border-gray-100 dark:border-gray-700 transition-all duration-300">
                                ...
                                <div id="slots-{{ $day }}" class="w-full">
                                    @php
                                        $oldSlots = old('week_schedule.'.$day, $weekSchedules[$day] ?? null);
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // --- Navigation et gestion des créneaux horaires par jour ---
        ...
        // Ajout/suppression de créneaux horaires
