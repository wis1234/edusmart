    ...
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">Edit Student</h1>
                    <p class="text-gray-500 dark:text-gray-300">Update student information</p>
                </div>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('students.show', $student) }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 font-semibold shadow hover:bg-gray-300 dark:hover:bg-gray-600 transition">
        ...
        <form action="{{ route('students.update', $student) }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-gray-800 shadow-md rounded-xl px-8 pt-6 pb-8 mb-4">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="first_name" id="first_name" value="{{ old('first_name', $student->first_name) }}" required maxlength="255"
                ...
                <div>
                    ...
                    <input type="text" name="last_name" id="last_name" value="{{ old('last_name', $student->last_name) }}" required maxlength="255"
                ...
                <div>
                    ...
                    <select name="class_room_id" id="class_room_id" data-current="{{ old('class_room_id', $student->class_room_id) }}"
                        ...
                        @foreach($classRooms as $classRoom)
                            <option value="{{ $classRoom->id }}" data-school="{{ $classRoom->school_id }}" {{ old('class_room_id', $student->class_room_id) == $classRoom->id ? 'selected' : '' }}>{{ $classRoom->name }} ({{ $classRoom->grade_level ?? '' }})</option>
                        @endforeach
                    </select>
                    ...
                    <select name="selected_parent_id" id="selected_parent_id"
                        ...
                        @foreach($users as $parent)
                            @if($parent->role === 'parent')
                                <option value="{{ $parent->id }}" {{ old('selected_parent_id', $student->selected_parent_id) == $parent->id ? 'selected' : '' }}>{{ $parent->first_name }} {{ $parent->last_name }}</option>
                            @endif
                        @endforeach
                    </select>
                ...
                <!-- Admission Number -->
                <div>
                    <label for="admission_number" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Admission Number <span class="text-red-500">*</span></label>
                    <input type="text" name="admission_number" id="admission_number" value="{{ old('admission_number', $student->admission_number) }}" required maxlength="255" readonly
                        class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-indigo-500 focus:border-indigo-500 @error('admission_number') border-red-500 @enderror">
                    @error('admission_number')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
                <!-- Roll Number -->
                <div>
                    <label for="roll_number" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Roll Number</label>
                    <input type="text" name="roll_number" id="roll_number" value="{{ old('roll_number', $student->roll_number) }}" readonly
                        class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-indigo-500 focus:border-indigo-500 @error('roll_number') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <input type="date" name="admission_date" id="admission_date" value="{{ old('admission_date', $student->admission_date ? $student->admission_date->format('Y-m-d') : '') }}" required
                ...
                <div>
                    ...
                    @if($student->profile_photo_url)
                        <img src="{{ $student->profile_photo_url }}" alt="Profile Photo" class="mb-2 w-24 h-24 object-cover rounded-full">
                    @endif
                ...
                <div>
                    ...
                    <input type="text" name="blood_group" id="blood_group" value="{{ old('blood_group', $student->blood_group) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="address" id="address" rows="2"
                        class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-indigo-500 focus:border-indigo-500 @error('address') border-red-500 @enderror">{{ old('address', $student->address) }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="medical_conditions" id="medical_conditions" rows="2"
                        class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-indigo-500 focus:border-indigo-500 @error('medical_conditions') border-red-500 @enderror">{{ old('medical_conditions', is_array($student->medical_conditions) ? implode(", ", $student->medical_conditions) : $student->medical_conditions) }}</textarea>
                ...
                <div>
                    ...
                    <select name="academic_year" id="academic_year" required
                        ...
                        @for ($year = $startYear; $year <= $endYear; $year++)
                            ...
                            <option value="{{ $academic }}" {{ old('academic_year', $student->academic_year) == $academic ? 'selected' : '' }}>{{ $academic }}</option>
                        @endfor
                    </select>
