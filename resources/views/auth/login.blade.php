...
<body class="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-100 via-white to-purple-100 dark:from-gray-900 dark:via-gray-800 dark:to-indigo-900">
    <div class="w-full max-w-md mx-auto p-8 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl">
        <div class="flex flex-col items-center mb-6">
            ...
            <h2 class="text-xl font-bold text-gray-900 dark:text-white">Sign in</h2>
            <p class="text-gray-500 dark:text-gray-300 text-sm">Please enter your credentials to access your account</p>
        </div>
        @if(session('status'))
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-6">
            ...
            <div>
                ...
                <div class="mt-1 relative">
                    <input id="password" name="password" type="password" autocomplete="current-password" required class="block w-full px-4 py-3 border border-gray-300 dark:border-gray-700 rounded-lg shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-900 dark:text-white">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    <input id="remember_me" name="remember" type="checkbox" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded dark:bg-gray-900 dark:border-gray-700">
                    <label for="remember_me" class="ml-2 block text-sm text-gray-900 dark:text-gray-300">Remember me</label>
                </div>
                @if(Route::has('password.request'))
                    <div class="text-sm">
                        <a href="{{ route('password.request') }}" class="font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300 transition">Forgot your password?</a>
                    </div>
                @endif
            </div>
        ...
        <div class="mt-6 text-center text-sm text-gray-600 dark:text-gray-400">
            ...
            @if(Route::has('register'))
                <a href="{{ route('register') }}" class="font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300 transition">Create an account</a>
            @endif
        </div>
    </div>
</body>
